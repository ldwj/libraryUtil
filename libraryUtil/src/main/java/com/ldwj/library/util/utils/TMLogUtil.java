...
/**
 * Log统一管理类 支持是否展示log 、默认tag
 */
public class TMLogUtil {

  private static boolean isDebug = false; //是否需要打印bug,可以在application的onCreate函数里面初始化
  private static String TAG = "TM_Android_Log";
  ...
  private static void handleMessage(int level, String tag, String message) {
//    TAG = tag;
    if (isDebug) {
      STRING_START = 0;
      STRING_END = 1000;
